...
export const getPostsForAuthUser = createAsyncThunk(
  "posts/getPostsForAuthUser",
  async ({ TOKEN }) => {
...
export const getOnePostForAuthUser = createAsyncThunk(
  "posts/getOnePostsForAuthUser",
  async ({ id, TOKEN }) => {
    const userRequest = axios.create({
      baseURL: BASE_URL,
      headers: { Authorization: `Bearer ${TOKEN}` },
...
// CURRENT USER POST
export const getCurrentUserPost = createAsyncThunk(
  ...
  extraReducers(reducers) {
    reducers
      ...
      .addCase(getPostsForAuthUser.fulfilled, (state, action) => {
        ...
        // console.log(action.payload);
        isFulfilled.allPostsForAuthUser = action.payload;
      })
...
export const { getLikedStatus, getErrorStatus, resetState } =
  getAuthUserPostsSlice.actions;
