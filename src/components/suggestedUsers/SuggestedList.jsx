...
const SuggestedList = () => {
    ...
    return (
        <div className='marginTop15 marginBtm15 width50 relative' style={{ height: '95%' }} >
            <h4 className='mainBackground white padding05rem' style={{ position: 'sticky', top: '0', zIndex: 5 }}>Suggested Users to Follow</h4>
                    ...
                    {data && data.map((images, index) => (
                        <Col xs={24} sm={12} lg={6} xl={6} key={index}>
                            <div className='white gap05rem mainBackground padding1rem flex column align_center margin05'>
                                <Link to={currentUser?.data?.id === images.id ? '/profile' : `/user/${images.name}/${images.id}/profile`}
                                    style={{
                                        color: '#fff',
                                        textDecoration: 'none'
                                    }}
                                ...
                                >
                                    <div>
                                        <img className='friendsImage' src={images.image} style={{ objectFit: 'cover' }} alt="" />
                                    </div>
                                    <div className='flex align_center gap05rem'>
                                        <p className='font12' style={{ fontFamily: 'sans-serif' }}>{images.name.length > 12 ? images.name.slice(0, 12) + '...' : images.name}</p>
