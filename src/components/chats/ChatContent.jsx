...
import { FaEllipsisV, FaPaperPlane, FaPlane } from 'react-icons/fa'
    ...
    return (
        <div className='chatContentMainDiv'>
            ...
            <div className='scrollBar chatContentMessagesContainer'>
                {
                    chats.map((message) => (
                        <div key={message.sender_id} className='padding05rem flex' style={{ flexDirection: currentUserId === message.sender_id ? 'row' : 'row-reverse' }}>
                            <div style={{ borderRadius: currentUserId === message.sender_id ? '0 8px 8px 16px' : '8px 0 16px 8px', }} className='padding05rem chatContentMessagesDiv'>
                                <p style={{ whiteSpace: 'pre-line', fontWeight: 600 }} className='font12'>{message.message}</p>
            ...
            <div className='chatContentInputDiv'>
                <TextArea style={{ resize: 'none' }} className='scrollBar chatContentInput' placeholder='Write a message........' />
